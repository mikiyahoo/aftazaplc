...
export class EmailService {
  ...
  constructor() {
    ...
    const fixedPassword = smtpPass ? this.escapePassword(smtpPass) : undefined;
    ...
    this.transporter = nodemailer.createTransport({
      ...
      auth: {
        ...
        pass: fixedPassword || process.env.SMTP_PASS,
      },
      tls: {
  ...
  /**
   * Escape special characters in password for SMTP authentication
   */
  private escapePassword(password: string): string {
    // Replace problematic characters
    return password
      .replace(/@/g, '%40')  // URL encode @ symbol
      .replace(/\$/g, '%24'); // URL encode $ symbol
  }
